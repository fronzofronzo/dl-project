"""Evaluate a trained Φ on the CelebA test benchmark, vs the frozen DB.

Reuses the shared eval harness (metrics / groundtruth / retrieval). Query-side
only: Φ composes v_q from the test reference + signed conditions, then cosine
against the frozen test DB. Same protocol as the baselines (restrict to the JSON
source keys, Recall@K / Precision@K hit-rate).

`eval_phi` is imported by train.py for periodic validation; `main` evaluates a
saved checkpoint and freezes a results table.

Run from repo root:  python -m src.solution_b.run
"""
import json
...
def eval_phi(phi, db, gts, attr_index, device, ks=KS):
    """Rank every valid source of every query through Φ; return evaluate_all rows.
...
def write_results(rows, name):
    """Freeze a per-query table (+ MACRO, + vs naive/clay if present)."""
    RESULTS.mkdir(exist_ok=True)
    (RESULTS / f"solution_b_{name}.json").write_text(json.dumps(rows, indent=2))

    cols = [f"{m}@{k}" for k in KS for m in ("recall", "precision")]
    ordered = [q for q in rows if q != "MACRO"] + (["MACRO"] if "MACRO" in rows else [])
    lines = [f"# Solution B — Φ = {name}", "",
    ...
    (RESULTS / f"solution_b_{name}.md").write_text("\n".join(lines) + "\n")
    print(f"frozen -> {RESULTS / f'solution_b_{name}.md'}")


def main(ckpt=RESULTS / "phi_mlp.pt", name="mlp"):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    db = load_db(DB_TEST).float().to(device)
    gts = build_ground_truth(EVAL_JSON)
    attr_index = attr_index_test()

    phi = MLPPhi().to(device)
